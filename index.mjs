...
async function generateProcessedCSV() {
  ...
  try {
    ...
    const csvRows = ["itemname,item_uuid,item_price,customization_title,option_title,option_price,option_uuid\n"];
          ...
          for (const customization of itemData.options) {
            ...
            if (customization.options && Array.isArray(customization.options)) {
              for (const option of customization.options) {
                ...
                csvRows.push(`"${itemname.replace(/"/g, '""')}","${itemUuid}",${itemPrice},"${customizationTitle.replace(/"/g, '""')}","${optionTitle.replace(/"/g, '""')}",${optionPrice || ''},"${optionUuid}"\n`);
                
                // Handle nested child customizations
                if (option.childCustomizationList && Array.isArray(option.childCustomizationList)) {
                  for (const childCustomization of option.childCustomizationList) {
                    ...
                    if (childCustomization.options && Array.isArray(childCustomization.options)) {
                      for (const childOption of childCustomization.options) {
                        ...
                        csvRows.push(`"${itemname.replace(/"/g, '""')}","${itemUuid}",${itemPrice},"${childTitle.replace(/"/g, '""')}","${childOptionTitle.replace(/"/g, '""')}",${childOptionPrice || ''},"${childOptionUuid}"\n`);
                      }
                    }
                  }
                }
    ...
    // Output first few rows as preview
    const preview = csvRows.slice(0, 10).join("");
    console.log("\n--- Preview of customizations.csv ---\n" + preview + "\n--- end of preview ---\n");
